...
<div class="container">
	...
	<a href="<?php echo U('fxList');?>" class="tja">等级设置</a>
	<div class="hdtright">
		<form action="" method="get">
		<span>用户：</span><input type="text" name="user_name" value="<?php echo ((isset($search['user_name']) && ($search['user_name'] !== ""))?($search['user_name']):''); ?>" class="inputt input1" />
		<span>等级：</span>
		<select name="fx_group" class="easyui-combobox" style="width:130px;height:25px">
			<option value="">全部</option>
			<?php $_result=get_fx_group();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($key) == $search['fx_group']): ?>selected="selected"<?php endif; ?>><?php echo ($val['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
...
<table width="910" border="0" cellspacing="1" cellpadding="0" class="table1 tab">
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			...
			<td align="center"><?php echo ($v['fx_sup']); ?></td>
			<td align="center"><img src="<?php echo ($v['user_avatar']); ?>" width="50" height="50"/></td>
			<td align="center"><?php echo ($v['fx_money']); ?></td>
			<td align="center"><?php echo ($v['fx_inter']); ?></td>
			<td align="center"><?php echo get_fx_group($v['fx_group'], 'name');?></td>
