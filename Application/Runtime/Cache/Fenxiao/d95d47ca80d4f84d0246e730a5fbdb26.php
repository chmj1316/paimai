        ...
        $.ajax({
            ...
            <div class="fxyj_main">
                <div class="fxyj_main_top">
                    <ol>
                        总收入：<?php echo ($user_info['fx_inter_lj']); ?><br>
                        当前：<?php echo ($user_info['fx_inter']); ?>，可兑换：<?php echo ($user_info['fx_inter']); ?>
                    </ol>
                    <ul><a href="javascript:alert('暂未开启');" title="">积分商城</a></ul>
                </div>
                <div class="fxyj_main_bottom">
                    <ol class="fix">
                        <li><a href="javascript:void(0);" class="xz">分销积分</a></li>
                        <li><a href="javascript:void(0);">积分明细</a></li>
                    </ol>
                    <ul>
                        <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li class="fix">
                                <p>
                                    返现积分：<em>￥<?php echo ($val['inter']); ?></em><br>
                                    返现状态：已返现<br>
                    ...
                    <ul style="display:none;">
                        <?php if(is_array($lists2)): $i = 0; $__LIST__ = $lists2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li class="fix">
                                <p>
                                    ...
                                    收支积分：<em>￥<?php echo ($val['inter']); ?></em><br>
                                    当前积分：<em>￥<?php echo ($val['at_inter']); ?></em><br>
